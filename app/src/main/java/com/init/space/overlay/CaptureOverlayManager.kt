package com.init.space.overlay
...
import com.init.space.utils.VoiceRecorder
...
class CaptureOverlayManager(private val context: Context) {
    ...
    private fun buildView() {
        val themed = ContextThemeWrapper(context, R.style.Theme_InitSpace)
    ...
    private fun flashIn() {
        ...
        ObjectAnimator.ofFloat(flashOverlay, "alpha", 0f, 0.15f, 0f).also {
            it.duration = 70
    ...
    private fun stopRecording() {
        ...
        if (dur < 500) {
            ...
            voiceTimerText.text = FileUtils.formatDigitalDuration(dur)
        }
    }
    ...
    private fun startRecordingTicker() {
        recordingTicker = object : Runnable {
            override fun run() {
                ...
                voiceTimerText.text = FileUtils.formatDigitalDuration(ms)
    ...
    private fun saveAndDismiss() {
        ...
        if (ssPath.isNullOrEmpty() || !isScreenshotReady) {
            Toast.makeText(context, "screenshot is processing...", Toast.LENGTH_SHORT).show()
            return
        }
        ...
        scope.launch(Dispatchers.IO) {
            try {
                AppDatabase.getDatabase(context).insert(
                    CaptureEntry(
    ...
    private suspend fun decodeBitmapWithRetries(uri: Uri): Bitmap? {
        repeat(8) { attempt ->
            val bitmap = try {
    ...
    private fun setSaveEnabled(enabled: Boolean) {
        ...
        btnSave.alpha = if (enabled) 1f else 0.45f
    }
    ...
    private fun updateVoiceBars(amplitude: Int) {
        ...
        voiceBars.forEachIndexed { index, bar ->
            val stagger = 0.2f * index
            val scale = (0.35f + normalized * (1.3f - stagger)).coerceAtLeast(0.35f)
            bar.animate()
                .scaleY(scale)
                .alpha((0.55f + normalized * 0.45f).coerceAtMost(1f))
                .setDuration(80L)
                .setInterpolator(linear)
                .start()
        }

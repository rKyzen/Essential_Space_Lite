package com.init.space.capture
...
import com.init.space.overlay.CaptureOverlayManager
import java.util.concurrent.Executor

object CaptureOrchestrator {
    // 50ms minimal hardware sync buffer to ensure key dispatch finishes before hardware frame capture
    private const val SCREENSHOT_BUFFER_DELAY_MS = 50L
    ...
    private var directScreenshotTask: Runnable? = null
    ...
    fun onTrigger(service: AccessibilityService) {
        ...
        fallbackStart = null
        directScreenshotTask?.let { mainHandler.removeCallbacks(it) }
        directScreenshotTask = null
        ...
        // Buffer delay ensures screen compositor is clean with no volume UI overlay
        directScreenshotTask = Runnable {
            requestDirectScreenshot(service, token)
        }.also {
            mainHandler.postDelayed(it, SCREENSHOT_BUFFER_DELAY_MS)
        }

        fallbackStart = Runnable {
            startFallbackCapture(service, token, triggerTime)
        }.also {
            mainHandler.postDelayed(it, SCREENSHOT_BUFFER_DELAY_MS + DIRECT_CAPTURE_GRACE_MS)
        }
    }
